<script>
  ...
  function handleKeydown({ keyCode }) {
      ...
      if (currentIndex === -1) {
          ...
          if (keyCode === 37) newIndex = (currentIndex + items.length - 1) % items.length
  ...
  function playerRenderer(item, isSelected) {
    ...
    return `
      ...
      <div class="w-8 h-8 rounded-full m-2 mr-4 border-2" style="background-color: ${α(item.color,.5)}; border-color: ${item.color}">
  ...
  function handlePlayerClick(player) {
    for (let i=0; i < matches.length; i++)
      data[player].matches[i].selected = !data[player].matches[i].selected;
    $selected = Array.from(new Set([...$selected, ...data[player].matches])).filter((x) => x.selected !== false);
    console.log("selected", $selected);
  }
  function handleMatchClick(player, match) {
  ...
  <div id="second-col">
    <div id="table">
      <div class="flex flex-row">
          ...
          {#if showSearch}
             <div class="py-1">
              <Svelecte {options} renderer="player-block" placeholder="Select Player" bind:value={value} valueAsObject={true} on:change={onChange}></Svelecte>
             </div>
          {:else}
					<Button color="gray" classes="text-md my-1 w-full" handleClick={() => {showSearch = !showSearch}}>
						<svg
            ...
            <div class="player" on:click={handlePlayerClick(player)}>
              <div class="flex flex-row flex-grow items-center">
                <div class="player-avatar">
                  <div class="avatar-img" style="background-color: {α(data[player].color,.5)}; border-color: {data[player].color}">
                    <img
                      class="rounded-full"
              ...
              <div class="items-stretch">
                ...
                handleClick={() => removePlayer(player)}><div class="ml-1.5 text-white">✕</div></Button>
                {/if}
              </div>
            </div>
          </div>
          <div class="metric-col rounded-md">
            ...
            {#each matches as match}
              <div
                ...
                tabindex="0"
                on:click={() => handleMatchClick(player, match)}
                class:selected={data[player].matches[match].selected}
              >
                <div class="flex flex-row mt-2 mb-2 px-2 w-full">
                  <div class="flex-grow text-xs text-gray-600">
                    {data[player].matches[match].tournamentInfo.month} {data[player].matches[match].tournamentInfo.year}
                  </div>
    ...
    {:else}
       <div class="h-full grid items-center text-center align-center">
         <div style="filter: brightness(165%) contrast(122%);">
          <img class="w-full object-fit px-16 rounded-2xl" src="./tennis.gif" alt=""/>
          <span class="text-xl text-gray-900 font-medium">Please select a match to analyze.</span>
         </div>
       </div>
    {/if}
    <div>
...
<style lang="postcss">
  ...
  .item {
    @apply transition-all flex items-center p-0 mx-1 rounded-md shadow-md border-2 border-gray-200 bg-white cursor-pointer;
    min-width: 150px;
    min-height: 150px;
    margin: 10px;
  }
  ...
  .item:focus,
  .item:active {
    @apply border-4 border-yellow-400 bg-yellow-50;
  }
  #vp {
